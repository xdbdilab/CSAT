...
# Data
We conduct experiments and obtain data on two cloud clusters and a [cloud server](https://www.aliyun.com/), where each cluster has consisted of three servers, and each server has four Intel$\circledR$ Core$^\text{TM}$Xeon CPU @ 2.50GHz and 8 GB RAM. The remaining one server is equipped with two Intel$\circledR$ Core$^\text{TM}$Xeon CPU @ 2.50GHz and 4 GB RAM. CDAT has been evluated on 8 real-world configurable software system:
...
To evaluate the performance improvement, we use the ***Impro***, which is computed as,

![1](http://latex.codecogs.com/svg.latex?{\rm{Impro}}(SUT,W) = \frac{P(C_o|SUT,W)-P(C_d|SUT,W)}{P(C_d|SUT,W)} \times 100\%)
...
In the table below, we use three different measurement constraints (i.e., 100, 200, 300) to evaluate the impact of measurement effort.  The results are obtained when evaluating CDAT on a Windows 10 computer with Intel(R) Core(TM) i7-8700 CPU @ 3.20GHz 16GB RAM.
